...
- **共有資源の書込みの部分だけではなく、読み込みの部分も含めてクリティカルセッションを保護すべき（最新の正しい値を読み込むことを保証するため）**
...
- `sync.Cond`は、条件変数を表す型で、以下のメソッドを持つ。
	...
	- `Signal()`: 待機中のゴルーチンのうち1つを起こす。
  	- 待機中のゴルーチンが複数ある場合、そのうちの1つをランダムに選択して起こす。
	- `Broadcast()`: 待機中のすべてのゴルーチンを起こす。
  	- ただ、Mutexを獲得できるのは1つのゴルーチン(goroutine)だけなので、他のゴルーチンはMutex獲得待ちのままである。