...
> DDD（ドメイン駆動設計）でよく登場する 3 つの概念を、初心者向けにまとめたもの。
> 関連: [DDD（ドメイン駆動設計）について](<../Coding-Design-Practices/ DDD（ドメイン駆動設計）について.md>)

## 全体像（先に結論）

3 つの概念は「役割」が違うだけで、対立するものではなく **組み合わせて使う** ものです。
...
flowchart TB
    subgraph proc["1つのプロセス（アプリ）の中"]
        ...
        event["OrderPlaced<br/>（注文された）<br/><b>← Domain Event</b>"]
...
> **図が表示されない場合**：VSCodeでMermaid図をプレビューするには
> `Markdown Preview Mermaid Support` などの拡張機能が必要です（GitHub上では標準で表示されます）。
...
**「ドメイン（業務領域）の中で発生した、ビジネス的に重要な出来事」を表すオブジェクト**です。
...
DDDには「**1トランザクション = 1集約（Aggregate）だけを更新する**」という原則があります。
（集約 = 「必ず一緒に整合性を保つべきデータのかたまり」。例：注文とその明細行）

すると「注文が確定したら在庫も減らしたい」のように、**複数の集約をまたいで処理したい** ケースで困ります。
これを解決するのが Domain Events です。

- 注文集約は「OrderPlaced が起きた」という事実だけを発行する。
- 在庫集約は後からそのイベントを受け取って、自分のトランザクションで在庫を減らす。
- こうして **集約どうしを疎結合（お互いを直接知らない）に保ったまま連携** できる。

これが「**結果整合性（Eventual Consistency）**」＝「今すぐ完全に一致していなくても、最終的には一致する」という考え方です。
...
**「In-Process」= 同じアプリケーションプロセスの中、メモリ上だけで完結する** という意味です。
...
Kafka や RabbitMQ は **別プロセス・別サーバーで動く「メッセージブローカー」** を経由します。
イベントはネットワークを通り、ディスクに保存されます。
...
この2つは **役割が完全に別物** です。よくある郵便のたとえで整理します。
...
**運ぶ仕組み（How）を差し替えても、メッセージ（What）は変えなくてよい** からです。
...
「DBを更新して、**かつ** 外部にも通知する」処理には、実は **落とし穴** があります。
...
この①と②は **別々のシステムへの書き込み** なので、まとめて1つのトランザクションにできません。
そのため、途中で失敗すると **不整合** が起きます。
...
これが **Dual Write問題** です。「2つの異なる書き込み先を、確実に両方成功／両方失敗にはできない」という根本的な難しさ。
...
**業務データの更新と outbox への書き込みを、1つのDBトランザクションにまとめる** のがミソです。
同じDBなので、これは1トランザクションで「両方成功 or 両方失敗」を保証できます。
...
これで Dual Write 問題は解消します。
「注文は保存されたのに通知が飛ばない」も「注文がないのに通知だけ飛ぶ」も、どちらも起こりません。

### メモリキュー（In-Process Event Bus）との違い — At-most-once vs At-least-once

先ほどの In-Process Event Bus は「メモリ上」でした。ここが信頼性の分かれ目です。
...
> Outbox は「重複することがある」ので、**受信側は「同じイベントを2回受け取っても結果が同じ」になるように作る**必要があります（＝冪等に作る）。
...
- **Domain Events** … 「〇〇が起きた」という業務上の事実を表す **メッセージ（What）**。DDDでは集約をまたぐ連携を疎結合に保つ手段。
- **In-Process Event Bus** … そのメッセージを **同一プロセス内・メモリ経由** で pub/sub 配信する **運搬の仕組み（How）**。速いが落ちると消える。Kafka等の分散バスはネットワーク＋永続化で別サービス間を確実に繋ぐ。
- **What と How は分離** すると、配送方法（Bus）を差し替えてもメッセージ（Event）を変えずに済み、疎結合・テスト容易になる。
- **Transactional Outbox** … 「DB更新」と「外部通知」を確実に整合させる設計。業務更新と outbox 書き込みを **1トランザクション** にまとめて **Dual Write問題** を解決。メモリキューが **At-most-once**（消えうる）なのに対し、Outbox は **At-least-once**（最低1回・要冪等）で信頼性が高い。
