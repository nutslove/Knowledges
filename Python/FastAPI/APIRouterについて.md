...
`tags` は **OpenAPI ドキュメント（自動生成される API ドキュメント）で、エンドポイントをグループ分けするためのラベル**。

FastAPI は自動的に `/docs`（Swagger UI）や `/redoc` という API ドキュメントを生成するが、`tags` を指定すると、そのドキュメント上で **エンドポイントがタグごとにまとまって表示** される。

```python
router = APIRouter(prefix="/users", tags=["users"])
```

とすると、`/docs` 画面で「**users**」という見出しの下に、そのルーターの全エンドポイント（`GET /users`、`POST /users` など）がグループ化されて並ぶ。

#### なぜ便利か

エンドポイントが増えてくると、ドキュメントが縦に長い一覧になって見づらくなる。`tags` で機能ごとに区切ることで、ドキュメントを読む人（フロントエンド開発者・API 利用者）が目的の API を探しやすくなる。

| タグ | 含まれるエンドポイント |
|------|----------------------|
| `users` | ユーザー関連（登録・取得・削除…） |
| `items` | 商品関連 |
| `auth` | 認証関連 |

#### 注意点

- **動作には一切影響しない**。あくまでドキュメントの見た目・整理のためだけのもの（パスのルーティングや認証などには無関係）。
- `include_router` 側でも `tags=["v1"]` を追加でき、その場合は **両方のタグが付与** される（`["users", "v1"]` のように両方の見出しに関連付く）。

> イメージとしては「Swagger UI の目次の章タイトル」。
...
##### 具体例: ボディから取得する（→ Pydanticモデル）

依存関数の引数を **Pydanticモデル** で宣言すると、その引数は **リクエストボディ（JSON）** から解決される（`skip: int` がクエリになるのと同じ「型注釈による自動解決」ルール）。ボディのバリデーションや共通の前処理を依存に切り出したいときに使う。
...
# ボディ(JSON)を Credentials として受け取り、認証して User を返す依存
def authenticate(creds: Credentials) -> dict:
    if creds.username != "alice" or creds.password != "secret":
...
async def login(user: dict = Depends(authenticate)):
    # authenticate() の戻り値がそのまま user に注入される
    return {"message": f"ようこそ {user['username']} さん", "role": user["role"]}
```

呼び出しの流れ（`POST /login` にボディ `{"username": "alice", "password": "secret"}` を送った場合）:

1. FastAPIが `Depends(authenticate)` を見て、`authenticate` のシグネチャ（`creds: Credentials`）を調べる
2. 引数の型が Pydanticモデルなので、**リクエストボディのJSONを `Credentials` にパース・バリデーション**する（フィールド不足や型不一致なら自動で **422**）
3. `authenticate(creds=...)` を呼ぶ。認証NGなら `HTTPException(401)` が送出され、本体は実行されない
4. 戻り値 `{"username": "alice", "role": "admin"}` を引数 `user` に注入する
5. `login` 本体が実行される

- ボディの受け取りとバリデーションは **型注釈（Pydanticモデル）だけで完結** し、`Request` から手で `await request.json()` する必要はない
- 認証・入力チェックといった前処理を依存に逃がせるので、複数のエンドポイントで **再利用** できる
- 同じボディをエンドポイント本体でも使いたい場合は、本体側にも `creds: Credentials` を宣言すればよい（FastAPIが同じボディを両方に渡す）
...
> [!NOTE]
> **`Depends` は「引数」に注入するだけで、レスポンスには関与しない**
>
...
戻り値が欲しいとき（現在のユーザーなど）は**引数**で受け取り、戻り値が不要な単なるガード（認証で弾くだけ）は **`dependencies=[...]`** に置く、と覚えるとよい。セクション3・8で出てきた `dependencies=[Depends(verify_token)]` は後者（認証ガード）。
