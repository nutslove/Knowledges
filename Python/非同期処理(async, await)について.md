...
- Python 3.7+で推奨される**エントリーポイント**のパターン

> [!NOTE]  
> `asyncio.run()`は **「ここから先は非同期の世界」** という境界を作るだけなので、外側の同期コードには影響を与えない。
...
- **`await`キーワードは「この操作が完了するまで待機し、その間は他のタスクを実行できる」ことを示す**
  - ただし、実際に制御がEventLoopに返されるのは**I/O待ちが発生する場合のみ**
  - I/O待ちがない即座に完了するコルーチンでは、制御を返さずそのまま続行する
...
### `await`の仕組み
`await`は一時停止の**可能性がある**ポイント。実際の動作は対象によって異なる：

| 状況 | 動作 |
|------|------|
| I/O待ちが発生する | 制御がEventLoopに返され、他のタスクを実行 |
| I/O待ちが発生しない | 制御を返さず、そのまま続行 |

**I/O待ちが発生する場合の流れ：**
```
コルーチンが await に到達（I/O待ちが発生）
      ↓
...
- **実行タイミング**：`gather()`は明示的に`await`するが、`TaskGroup`は`async with`ブロック終了時に内部で`await`される
  - `tg.create_task()` → タスクを登録（まだ実行されない）
  - `async with`ブロック終了時 → 内部の`__aexit__`で全タスクの完了を`await`
  ```python
  # async with asyncio.TaskGroup() as tg: は以下と同等
  ...
  try:
      ...
      await tg.__aexit__(...)  # ここで全タスク完了を待つ
  ```
```python
async def main():
    try:
