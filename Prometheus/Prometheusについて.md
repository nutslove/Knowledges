...
- exporterなどmetricを持つものがrestartするとzeroにリセット（reset）される
- `rate()`と`increase()`関数はmetric(値)のリセット（reset）を検出してうまく処理してくれるらしい

> [!NOTE]
> ## Counter Reset検出のメカニズム
> Counterは「単調増加する（増えるだけで減らない）」という前提で設計されている。この前提があるからこそ、Prometheusは**値が前回より減少した＝リセットが起きた**と自動判定できる。
>
> ### 検出の仕組み
> Prometheusがスクレイピングで取得した連続するサンプルを比較し、**現在の値が前回の値より小さければ、その間にcounter resetが起きたと判断する**。
>
> ### `rate()` / `increase()` の補正ロジック
> リセットを検出した場合、**リセット後の値をそのまま増分として加算する**ことで補正する。つまり「リセット直前の値 → 0 → リセット後の値」という流れにおいて、リセット後の値はゼロからの増分とみなされる。
>
> 具体例:
> ```
> スクレイプ1: 100
> スクレイプ2: 150  → 増分 = 150 - 100 = 50（通常）
> スクレイプ3: 200  → 増分 = 200 - 150 = 50（通常）
> スクレイプ4:  30  → 値が減った！リセット検出 → 増分 = 30（ゼロからの増加とみなす）
> スクレイプ5:  80  → 増分 = 80 - 30 = 50（通常）
>
> increase() の合計 = 50 + 50 + 30 + 50 = 180
> ```
>
> ※ 本来の真の増分は「200（リセット前の最終値）+ 80（リセット後の最新値）= 280」かもしれないが、**リセット直前からリセット発生までの間のスクレイプされなかった増分は失われる**。上の例だと、スクレイプ3（200）からリセットまでの間にさらにカウントが増えていた可能性があるが、その分は検出できない。
>
> ### リセット検出に関する注意点
> - **Counterを手動でリセットしてはいけない**: アプリケーション内部でCounterの値をゼロに戻すと、意図しないリセット検出が発生し、`rate()` の結果がおかしくなる。Counterは「増えるだけ」に徹する。
> - **rawなCounter値を直接グラフ化しない**: Counter値をそのままグラフにすると、リセット時に急落する見えにくいグラフになる。常に `rate()` や `increase()` を通して可視化する。
> - **`resets()` 関数でリセット回数を確認できる**: デバッグ用途で `resets(http_requests_total[1h])` とすると、指定期間内にリセットが何回発生したかが分かる。
> - **range幅はscrape intervalの少なくとも4倍以上にする**: `rate(xxx[15s])` のようにscrape間隔ギリギリの幅だとサンプル不足でリセット検出が効かなかったり、値が欠落する可能性がある。scrape_interval が15秒なら `rate(xxx[1m])` 以上にする。
>
> ### `rate()` と `increase()` と `irate()` の違い
> | 関数 | 返す値 | 用途 |
> |---|---|---|
> | `rate()` | 1秒あたりの平均増加率 | ダッシュボード、アラート向き。滑らかなグラフになる |
> | `increase()` | 指定期間の増加量の合計 | 「直近1時間で何件」のような絶対数を見たいとき |
> | `irate()` | 直近2サンプルだけで計算した瞬間増加率 | 高解像度グラフ向き。アラートには不向き |
>
> いずれもcounter reset検出と補正を内蔵しているため、プロセス再起動を跨いでも正しく計算される。
>
> ```promql
> # rate と increase の関係（近似的に等しい）
