...
> [!TIP]  
> - **TF（Term Frequency）**
>   - その単語が**1つの**ドキュメント内にどの程度出現するか
> - **IDF（Inverse Document Frequency）**
>   - その単語が**全**ドキュメントの中でどれだけ珍しいか
>
> 例えば、全体で1000個のドキュメントがあるとした場合、  
> - 例1: 「猫」という単語
>   ```
>   「猫」の統計:
>     - Doc1内での出現: 10回 → TF高い ✓
>     - 全体での出現: 1000個中10個のドキュメントのみ → IDF高い（希少）✓
>   ```  
>   - 結果: TF × IDF = 高スコア 🎯
>     - Doc1は「猫」について詳しく書いている（TF高）
>     - かつ「猫」は珍しい単語（IDF高）
>     - → 関連性が高い！
>
> - 例2: 「です」という単語
...
>   「です」の統計:
>     - Doc1内での出現: 10回 → TF高い ✓
>     - 全体での出現: 990個のドキュメント → IDF低い（ありふれている）✗
>   ```
>   - 結果: TF × IDF = 低スコア 📉
>     - Doc1には「です」が多い（TF高）
>     - でも「です」はどこにでもある（IDF低）
...
>
> TF = (Doc内での単語の出現回数)
> IDF = log(全ドキュメント数 / その単語を含むドキュメント数)
>
> 例1「猫」:
>   TF = 10
...
1. **スコア計算**
   ...
   - **クエリ中の単語ごとの寄与を合計して、ドキュメント全体に対するスコアを算出する（単語単位でスコアリング → ドキュメント単位で合計）**。例えば： 
     ```
     クエリ「機械学習 Python」の場合：
...
2. **ランキング（結果の取得）**
   ...
   - BM25は **「クエリの単語を多く含み、かつ希少な単語を含む文書」** を **高くスコアリングする** 仕組み
