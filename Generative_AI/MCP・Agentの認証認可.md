...
**Confused Deputy (混乱した代理人)** とは、正当な権限を持つ仲介者 (deputy) が、他者のリクエストを代理で処理する際に、その権限を悪用されてしまう問題。
MCPの文脈では、ユーザーから正規のトークンを受け取ったMCPサーバーが、そのトークンを下流APIにそのまま横流しすることで「MCPサーバーの権限を借りた不正アクセス」が成立してしまうケースを指す。
これを防ぐために、下流API呼び出し時は **Token Exchange (RFC 8693)** などでトークンを変換するのが推奨される。

### 「下流API (downstream API)」とは

MCPサーバー（やAgent）が処理を完結させるために、**その先で呼び出す別のAPI・サービス**のこと。MCPサーバーは「窓口」であって、実際のデータ取得や操作は先にある本物のサービスに対して行われることが多い。
...
[MCPサーバー]          ← 窓口
      ↓ ②さらに別のAPIを呼ぶ
[下流API]              ← Loki, Grafana, Thanos/Tempo, AWS API, GitHub API, 社内DB など
```

- **上流 (upstream)**: リクエストの発信元に近い側 (ユーザー、Client Agent)
- **下流 (downstream)**: リクエストが流れていく先 (MCPサーバーが呼ぶ実サービス)
