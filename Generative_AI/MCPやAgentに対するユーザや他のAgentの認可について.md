...
Googleが2025年4月に発表、現在Linux Foundation配下で標準化が進んでいる。
MCPが「Agent ↔ Tool」なのに対し、A2Aは「Agent ↔ Agent」の通信。

## コアコンセプト: Agent Card

各AgentはJSONの「名刺」を `/.well-known/agent.json` で公開する。
...
- **Skill単位の認可**: Agent Cardには `skills` 配列があり、各skillに必要なscopeを紐付ける
...
2026年5月時点では、Envoy AI Gatewayの **A2Aネイティブ対応は限定的**。
MCPサポートは v0.5 で本格化しているが、A2Aは「HTTPとして通せばJWT検証はできる」レベル。
A2A固有のskill単位RBACのような深い制御は別ツールに任せる形になる。

## A2Aネイティブ対応ゲートウェイ

| ツール | 特徴 |
|-------|-----|
| **agentgateway (Solo.io / kgateway)** | Rust製、MCP/A2A両方をネイティブ理解。Envoyベースだが別プロジェクト |
...
2. **ext_proc (リクエスト/レスポンス処理中)** — JSONボディの中身を見て判定や書き換え。MCP/A2AはJSON-RPCなのでこっちが重要
3. **Lua/Wasm filter** — 軽量な変換やヘッダ操作
4. **Application層** — MCPサーバー自身がOPAをライブラリとして埋め込む

A2Aの「skill単位の認可」やMCPの「tool単位の認可」は、リクエストボディの中の `method` や `tool_name` を見ないと判定できないので、**ext_procが本質的に必要**。
...
3. **フェーズ3**: A2Aネイティブ対応が必要なら
   - agentgateway (Solo.io / kgateway) への移行検討
