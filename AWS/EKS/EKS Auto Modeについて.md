...
- ENIにはスロット数の制限があり（インスタンスタイプによって異なる）、
  従来のセカンダリ(Secondary)IP方式では、1スロット = 1 Podだったが、
  Prefix Delegation方式では1スロットに/28プレフィックス（16個のIP）が
  割り当てられるため、1スロットあたり最大16 Podを配置できるようになる
  - 例：m5.large（10スロット/ENI）の場合
    - セカンダリIPモード: 1 ENI = 最大10 Pod
    - Prefix Delegation: 1 ENI = 最大160 Pod
  - EC2インスタンスタイプごとにENIの最大数とスロット数が決まっている
    - https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/AvailableIpPerENI.html  
      > Each instance type supports a maximum number of network interfaces, maximum number of private IPv4 addresses per network interface, and maximum number of IPv6 addresses per network interface.
...
> │ AWSのENI（1 ENI = 複数 IP）                                   │
> │                                                             │
> │   [ENI] ── 10.0.1.10  ← プライマリIP（メインのIP）              │
> │        ├─ 10.0.1.50  ← セカンダリIP（追加のIP）                 │
> │        ├─ 10.0.1.51  ← セカンダリIP                           │
> │        └─ 10.0.1.52  ← セカンダリIP                           │
> └─────────────────────────────────────────────────────────────┘
> ```
> - EKSでは各PodにVPCのIPアドレスを割り当てる必要がある。でもEC2インスタンスに付けられるENIの数は限られている。そこで、1つのENIに複数のIPを割り当て、それをPodに配る仕組みを使う。
> ```
> ┌─────────────────────────────────────────────────────────────┐
> │ EC2ワーカーノード                                             │
> │                                                             │
> │   [ENI]                                                     │
> │    ├─ 10.0.1.10（プライマリIP）→ ノード自身が使う                │
> │    ├─ 10.0.1.50（セカンダリIP）→ Pod Aに割り当て                │
> │    ├─ 10.0.1.51（セカンダリIP）→ Pod Bに割り当て                │
> │    └─ 10.0.1.52（セカンダリIP）→ Pod Cに割り当て                │
> └─────────────────────────────────────────────────────────────┘
> ```
> 
> ### Prefix Delegationとは
> - セカンダリIPを「1個ずつ」ではなく「16個まとめて」割り当てる機能
> ```
> ┌─────────────────────────────────────────────────────────────┐
> │ セカンダリIPモード（従来）                                      │
> │                                                             │
> │   ENIのスロットに「1個ずつ」IPを割り当てる                        │
> │                                                             │
> │   [スロット1] → 10.0.1.50（1個）                              │
> │   [スロット2] → 10.0.1.51（1個）                              │
> │   [スロット3] → 10.0.1.52（1個）                              │
...
> │                                                             │
> │   ENIのスロットに「/28プレフィックス（16個のIPブロック）」          │
> │   を割り当てる                                                │
> │                                                             │
> │   [スロット1] → 10.0.1.64/28（16個: .64〜.79）                 │
> │   [スロット2] → 10.0.1.80/28（16個: .80〜.95）                 │
> │   [スロット3] → 10.0.1.96/28（16個: .96〜.111）                │
> │                                                             │
> │   → 3スロット使用 = 48個のIP = 最大48 Pod                      │
> └─────────────────────────────────────────────────────────────┘
> ```
